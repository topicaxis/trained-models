    ...
    def _load_dataset(self, selected_categories, min_text_length):
        ...
        with open(self._dataset_file) as f:
            for line in f:
                ...
                if self._can_use_data(data, selected_categories, min_text_length):
                    documents.append(data["text"])
                    categories.append(data["category"])

        return documents, categories

    def create_train_test_dataset(self, test_size=0.3, min_text_length=200, selected_categories=None):
        documents, categories = self._load_dataset(selected_categories, min_text_length)
